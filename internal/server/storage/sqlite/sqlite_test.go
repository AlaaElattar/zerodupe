...
func TestLoginUser(t *testing.T) {
	t.Run("Test LoginUser returns user for correct credentials", func(t *testing.T) {
		...
		retrievedUser, err := sqliteStorage.LoginUser("test", "test")
		assert.NoError(t, err)
		assert.Equal(t, user.Username, retrievedUser.Username)
		assert.Equal(t, user.Password, retrievedUser.Password)
		assert.Equal(t, user.Salt, retrievedUser.Salt)
	...
	t.Run("Test LoginUser returns error for incorrect password", func(t *testing.T) {
		sqliteStorage := NewSqliteStorageMock()
		_, err := sqliteStorage.LoginUser("test", "wrong")
		assert.Error(t, err)
